...
一个面向 Java 开发者的面试刷题平台，包含 Vue Web 前端、Spring Boot 后端和原生 Android WebView 客户端。管理员可以上传知识文档并通过 Codex CLI 自动生成、校验和入库单选题；普通用户可以按标签、难度和练习模式刷题，并使用错题本、收藏和学习统计功能。
...
| 学习闭环 | 答题解析、错题本、收藏、已答题记录、每日与标签统计 |
...
```mermaid
flowchart LR
    User[Web / Android 用户] --> Frontend[Vue 多页面前端]
...
- 前端：Vue 3、Vite 8、Markdown-It、Lucide Vue。
